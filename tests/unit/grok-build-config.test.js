...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("polyrouter");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "polyrouter-general-purpose",
      explore: "polyrouter-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.polyrouter\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.polyrouter-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.polyrouter-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# polyrouter-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.polyrouter-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("polyrouter-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.polyrouter-");
    expect(reset).not.toContain("polyrouter-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("polyrouter-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("polyrouter-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("polyrouter-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("polyrouter-explore");
    expect(getGrokSubagentSlot("plan")).toBe("polyrouter-plan");
