...
Despliega PolyRouter en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.polyrouter` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/polyrouter
sudo chown $USER:$USER /var/lib/polyrouter
```
...
Crea `/etc/nginx/sites-available/polyrouter`:
...
sudo ln -s /etc/nginx/sites-available/polyrouter /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de PolyRouter
...
# Actualizar PolyRouter
cd /path/to/polyrouter/app
git pull
npm install
npm run build
pm2 restart polyrouter
...
tar -czf polyrouter-backup-$(date +%Y%m%d).tar.gz /var/lib/polyrouter

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/polyrouter-$(date +\%Y\%m\%d).tar.gz /var/lib/polyrouter
```
...
```bash
# Verificar si PolyRouter está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/polyrouter
chmod 755 /var/lib/polyrouter
```
