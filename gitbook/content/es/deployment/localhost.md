...
Ejecuta PolyRouter en tu máquina local para desarrollo y uso personal.

---

## 📦 Instalación

Instala PolyRouter globalmente vía npm:
...
Inicia PolyRouter con un solo comando:
...
- **Directorio de datos**: `~/.polyrouter`
...
```bash
DATA_DIR=/path/to/data polyrouter
```
...
Presiona `Ctrl+C` en la terminal donde PolyRouter se está ejecutando.

```bash
# En la terminal ejecutando polyrouter
^C  # Presiona Ctrl+C
```
...
```bash
# Respaldo
cp -r ~/.polyrouter ~/.polyrouter.backup

# Restaurar
cp -r ~/.polyrouter.backup ~/.polyrouter
```
