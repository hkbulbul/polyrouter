...
PolyRouterに関する一般的な質問。

---

## PolyRouterとは?

**PolyRouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、PolyRouter自体は100%無料でオープンソースです。**
...
**PolyRouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## PolyRouterをセルフホストできますか?

**はい! PolyRouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.polyrouter`)
...
**はい、PolyRouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.polyrouter`(またはカスタム`DATA_DIR`)にローカル保存
- PolyRouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/hkbulbul/polyrouter/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.polyrouter`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/hkbulbul/polyrouter/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [useusepolyrouter.com/docs](https://useusepolyrouter.com/docs)
