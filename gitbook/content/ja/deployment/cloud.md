...
リモートアクセスと本番利用のため、VPSまたはDockerにPolyRouterをデプロイ。
...
| `DATA_DIR` | `~/.polyrouter` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/polyrouter` を作成:
...
# リバースプロキシを使用しない場合、PolyRouterポートを許可
