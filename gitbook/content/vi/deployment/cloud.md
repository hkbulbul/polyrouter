...
Triển khai PolyRouter trên VPS hoặc Docker để truy cập từ xa và dùng trong production.
...
| `DATA_DIR` | `~/.polyrouter` | Đường dẫn lưu database và data |
...
Tạo `/etc/nginx/sites-available/polyrouter`:
...
# If NOT using reverse proxy, allow PolyRouter ports
...
```bash
# Check if PolyRouter is running
pm2 status
