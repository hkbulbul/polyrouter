...
const OAUTH_TEST_CONFIG = {
  ...
  github: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    extraHeaders: { "User-Agent": "PolyRouter", "Accept": "application/vnd.github+json" },
  },
  iflow: {
    ...
    switch (connection.provider) {
      ...
      case "zenmux": {
        const res = await fetchWithConnectionProxy("https://zenmux.ai/api/v1/chat/completions", {
          ...
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${connection.apiKey}` },
          body: JSON.stringify({
            model: connection.defaultModel || "deepseek/deepseek-v4-flash-free",
            messages: [{ role: "user", content: "ping" }],
            max_tokens: 1,
            stream: false,
          }),
      ...
      case "explabs":
      case "experiential": {
        const res = await fetchWithConnectionProxy("https://api.experientiallabs.ai/v1/models", {
          headers: { Authorization: `Bearer ${connection.apiKey}` },
        }, effectiveProxy);
        const valid = res.status !== 401 && res.status !== 403;
        return { valid, error: valid ? null : "Invalid API key" };
