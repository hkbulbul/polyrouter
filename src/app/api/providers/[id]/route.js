...
} from "@/lib/webCookieBrowserSignin.js";
...
export async function GET(request, { params }) {
  try {
    ...
    return NextResponse.json({ connection: sanitizeProviderConnection(connection) });
  } catch (error) {
  ...
  try {
    ...
    if (
      ...
      updateData.providerSpecificData = {
        ...(existing.providerSpecificData || {}),
        ...(stripManagedProviderData(providerSpecificData) || {}),
      };
    ...
    return NextResponse.json({ connection: sanitizeProviderConnection(updated) });
  } catch (error) {
...
export async function DELETE(request, { params }) {
  try {
    ...
    await removeManagedBrowserProfile(existing);
    ...
    if (error instanceof WebCookieSigninError) {
