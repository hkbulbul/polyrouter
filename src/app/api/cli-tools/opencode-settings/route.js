...
const hasPolyRouterConfig = (config) => {
  ...
  return !!config.provider["polyrouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["polyrouter"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("polyrouter/") ? config.model.replace(/^polyrouter\//, "") : null,
...
// POST - Apply PolyRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_polyrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing polyrouter provider entry and its models
    const existingProvider = config.provider["polyrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["polyrouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `polyrouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `polyrouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("polyrouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove PolyRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["polyrouter"]?.models) {
      delete config.provider["polyrouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["polyrouter"].models).length === 0) {
        delete config.provider["polyrouter"];
        if (config.model?.startsWith("polyrouter/")) delete config.model;
      } else if (config.model === `polyrouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["polyrouter"].models);
        config.model = `polyrouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire polyrouter provider
      if (config.provider) delete config.provider["polyrouter"];
      if (config.model?.startsWith("polyrouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("polyrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "PolyRouter settings removed from OpenCode",
    });
  } catch (error) {
