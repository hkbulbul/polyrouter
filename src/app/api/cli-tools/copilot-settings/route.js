...
const hasPolyRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "PolyRouter");
};

const getPolyRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "PolyRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getPolyRouterEntry(config);
...
// POST - Apply PolyRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing PolyRouter entry or append
    const idx = config.findIndex((e) => e.name === "PolyRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove PolyRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "PolyRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "PolyRouter removed from Copilot config",
    });
  } catch (error) {
