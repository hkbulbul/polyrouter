...
const hasPolyRouterConfig = (globalState) => {
  if (!globalState) return false;
  const isOpenAi =
    globalState.actModeApiProvider === "openai" || globalState.planModeApiProvider === "openai";
  const baseUrl = globalState.openAiBaseUrl || "";
  return isOpenAi && (baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("polyrouter"));
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasPolyRouter: hasPolyRouterConfig(globalState),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "PolyRouter settings removed from Cline" });
  } catch (error) {
