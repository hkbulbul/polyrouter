...
// Check if settings has PolyRouter config
const hasPolyRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["polyrouter"];
};

// Read per-agent models.json and return current model id (without "polyrouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["polyrouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasPolyRouter: hasPolyRouterConfig(settings),
...
// POST - Update PolyRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `polyrouter/${model}`;

    // Remove all old polyrouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("polyrouter/"))
    ...
    // Add fresh polyrouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`polyrouter/${m}`] = {};
    });

    // Remove old polyrouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("polyrouter/")) {
    ...
    // Update models.providers.polyrouter with all models
    settings.models.providers["polyrouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `polyrouter/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove PolyRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["polyrouter"];
    ...
    // Remove polyrouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("polyrouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses polyrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("polyrouter/")) {
