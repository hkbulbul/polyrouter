...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-polyrouter.env");
};
...
const hasPolyRouterConfig = (config) => {
  ...
  if (providers["polyrouter"]) return true;
...
export async function GET() {
  ...
  const hasPolyRouter = hasPolyRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_POLYROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile polyrouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["polyrouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_POLYROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "polyrouter configuration removed from jcode",
    });
  } catch (error) {
