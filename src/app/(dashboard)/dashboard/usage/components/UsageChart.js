...
export default function UsageChart({ period = "7d" }) {
  ...
  return (
    <Card className="flex min-w-0 flex-col gap-3 p-3 sm:p-4">
      <div className="grid w-full grid-cols-2 items-center gap-1  border border-border bg-bg-subtle p-1 sm:w-auto sm:self-start">
        <button
        ...
        <button
          ...
          className={`px-3 py-1  text-sm font-medium transition-colors ${viewMode === "cost" ? "bg-primary text-white shadow-sm" : "text-text-muted hover:text-text hover:bg-bg-hover"}`}
        >
          Cost
        </button>
      </div>
