...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    // Check for any PolyRouter model entry (support multi-model: custom:PolyRouter-0, custom:PolyRouter-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:PolyRouter"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:PolyRouter"))
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels);
      } else {
        // Legacy: single model stored as custom:PolyRouter-0
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:PolyRouter-0");
        if (legacy?.model) {
                ...
                <div className="p-4 bg-surface border border-border ">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="block px-3 py-2 bg-black/5 dark:bg-white/5  font-mono text-xs">curl -fsSL https://app.factory.ai/cli | sh</code>
                    </div>
                    <p className="text-text-muted">After installation, run <code className="px-1 bg-black/5 dark:bg-white/5 ">droid</code> to verify.</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:PolyRouter"))?.baseUrl && (
                  ...
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    {modelList.length > 0 && (
                      <div className="flex flex-col gap-0.5 mb-1">
                        {modelList.map((id) => (
                          <div key={id} className="flex items-center gap-1.5 px-2 py-1 bg-bg-secondary  border border-border">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className={`px-2 py-1.5  border text-xs shrink-0 ${hasActiveProviders ? "bg-surface border-border hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}
                      >
                        Select
                      </button>
                      <button onClick={addModel} disabled={!modelInput.trim()} className="px-2 py-1.5  border bg-surface border-border hover:border-primary text-xs shrink-0 disabled:opacity-50" title="Add model">
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasPolyRouter} loading={restoring}>
