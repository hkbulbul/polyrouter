...
export default function MitmServerCard({ apiKeys, cloudEnabled, onStatusChange }) {
    ...
    try {
      ...
      if (action === "trust-cert") {
        ...
        const keyToUse = selectedApiKey?.trim()
          ...
          || (!cloudEnabled ? "sk_polyrouter" : null);
        res = await fetch("/api/cli-tools/antigravity-mitm", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-1 text-xs text-text-muted" data-i18n-skip="true">
              {[
                ...
                <span key={label} className={`flex items-center gap-0.5 px-1.5 py-0.5  ${ok ? "text-green-600" : "text-text-muted"}`}>
          ...
          <div className="px-2 py-2  bg-surface/50 border border-border/50 flex flex-col gap-2">
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">Purpose:</span> Use Antigravity IDE & GitHub Copilot → with ANY provider/model from PolyRouter
            </p>
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">How it works:</span> Antigravity/Copilot IDE request → DNS redirect to localhost:443 → MITM proxy intercepts → PolyRouter → response to Antigravity/Copilot
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
              <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">PolyRouter Base URL</span>
              ...
              <input
                ...
                disabled={isRunning}
                className="flex-1 min-w-0 px-2 py-1.5 bg-surface  border border-border text-xs text-text-main focus:outline-none focus:ring-1 focus:ring-primary/50 disabled:opacity-50"
              />
            </div>
            {!isRunning && (
              <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                ...
                <input
                  ...
                  placeholder={cloudEnabled ? "Enter or pick API key" : "sk_polyrouter (default)"}
                  className="flex-1 min-w-0 px-2 py-1.5 bg-surface  border border-border text-xs text-text-main focus:outline-none focus:ring-1 focus:ring-primary/50"
                />
                {apiKeys?.length > 0 && (
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center" data-i18n-skip="true">
            {status?.certExists && !status?.certTrusted && (
              <button
                ...
                disabled={loading}
                className="flex w-full items-center justify-center gap-1.5  border border-yellow-500/30 bg-yellow-500/10 px-4 py-2 text-xs font-medium text-yellow-600 transition-colors hover:bg-yellow-500/20 disabled:opacity-50 sm:w-auto sm:py-1.5"
              >
            ...
            {isRunning ? (
              <button
                ...
                disabled={loading}
                className="flex w-full items-center justify-center gap-1.5  border border-red-500/30 bg-red-500/10 px-4 py-2 text-xs font-medium text-red-500 transition-colors hover:bg-red-500/20 disabled:opacity-50 sm:w-auto sm:py-1.5"
              >
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-1.5  border border-primary/30 bg-primary/10 px-4 py-2 text-xs font-medium text-primary transition-colors hover:bg-primary/20 disabled:opacity-50 sm:w-auto sm:py-1.5"
              >
          ...
          {/* Action error */}
          {actionError && (
            <div className="flex items-start gap-2 px-2 py-1.5  text-xs bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20">
          ...
          {serverIsWindows && !isAdmin && (
            <div className="flex items-center gap-2 px-2 py-1.5  text-xs bg-red-500/10 text-red-600 border border-red-500/20">
              <span className="material-symbols-outlined text-[14px]">shield_lock</span>
              <span>Administrator required — restart PolyRouter as Administrator to use MITM</span>
            </div>
          )}
        </div>
      </Card>
      ...
      {port443Conflict && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="mx-4 flex w-full max-w-md flex-col gap-4  border border-border bg-surface p-5 shadow-xl sm:p-6">
