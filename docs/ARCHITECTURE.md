# PolyRouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

PolyRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[PolyRouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.polyrouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.polyrouter/usage.json`, `~/.polyrouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.polyrouter/db.json`)
- usage stats: `~/.polyrouter/usage.json`
- request log lines: `~/.polyrouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[PolyRouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.polyrouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/polyrouter && npm run build`
- Build Docker image: `cd /root/dev/polyrouter && docker build -t polyrouter .`
