---
name: polyrouter-web-fetch
description: Fetch URL → markdown / text / HTML via PolyRouter /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---

# PolyRouter — Web Fetch
...
```bash
curl $POLYROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$POLYROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $POLYROUTER_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $POLYROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $POLYROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://useusepolyrouter.com","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.POLYROUTER_URL}/v1/web/fetch`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.POLYROUTER_KEY}`, "Content-Type": "application/json" },
