---
name: polyrouter-web-search
description: Web search via PolyRouter /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---

# PolyRouter — Web Search
...
```bash
curl $POLYROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$POLYROUTER_URL/v1/models/info?id=tavily/search"
```
...
```js
const r = await fetch(`${process.env.POLYROUTER_URL}/v1/search`, {
  method: "POST",
