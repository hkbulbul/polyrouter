---
name: polyrouter-stt
description: Speech-to-text via PolyRouter /v1/audio/transcriptions using OpenAI Whisper / Groq / Gemini / Deepgram / AssemblyAI / NVIDIA / HuggingFace models. Use when the user wants to transcribe audio, convert speech to text, or get subtitles from audio files.
---

# PolyRouter — Speech-to-Text
...
```bash
curl $POLYROUTER_URL/v1/models/stt | jq '.data[].id'
# Per-model params (language, response_format, prompt, temperature support)
curl "$POLYROUTER_URL/v1/models/info?id=openai/whisper-1"
```
...
`POST $POLYROUTER_URL/v1/audio/transcriptions` (OpenAI Whisper compatible, `multipart/form-data`)
...
```bash
curl -X POST "$POLYROUTER_URL/v1/audio/transcriptions" \
...
const r = await fetch(`${process.env.POLYROUTER_URL}/v1/audio/transcriptions`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.POLYROUTER_KEY}` },
  body: form,
});
